...
import {
    ...
    Platform, Image, Modal, StatusBar, Alert, Dimensions, ActionSheetIOS
} from "react-native";
...
import {
    collection, addDoc, query, onSnapshot, orderBy, serverTimestamp, doc, updateDoc, setDoc,
    writeBatch, arrayUnion, deleteDoc, getDocs
...
export default function ChatScreen() {
    ...
    // Оновлення часу останнього прочитання
    useEffect(() => {
    ...
    // Підписка на повідомлення та статус "прочитано"
    useEffect(() => {
    ...
    const pickImageAndSend = async () => {
        ...
        try {
            ...
            if (res.ok) {
                await addDoc(collection(db, "events", eventId, "messages"), {
                    userId: authUser?.uid, authorName: currentUserName,
                    createdAt: serverTimestamp(), readBy: [authUser?.uid],
                    type: "image", imageUrl: json.secure_url
                });
            }
        } catch (e) {
    ...
    const renderMessageItem = ({item, index}: { item: any, index: number }) => {
        ...
        const isSameAuthor = prevMessage?.userId === item.userId && prevMessage?.type !== "system";
                ...
                >
                    ...
                    <View style={styles.msgFooter}>
                        <Text style={[styles.msgTime, isMine && {color: 'rgba(255,255,255,0.7)'}]}>
                            {item.createdAt ? new Date(item.createdAt.toDate?.() || item.createdAt).toLocaleTimeString([], {
                                hour: '2-digit',
                                minute: '2-digit'
                            }) : ""}
                        </Text>
    ...
    return (
        <View style={styles.mainContainer}>
            ...
            {/* Pinned Msg */}
            {pinnedMsg && (
                <TouchableOpacity style={styles.pinnedContainer} activeOpacity={0.8}
            ...
            <KeyboardAvoidingView style={{flex: 1}} behavior={Platform.OS === "ios" ? "padding" : undefined}>
                <FlatList
                    ref={listRef} data={messages} renderItem={renderMessageItem}
                ...
                <View style={{backgroundColor: COLORS.white}}>
                    ...
                    <View style={styles.footerContainer}>
                        <View style={styles.inputRow}>
                            <TouchableOpacity style={styles.attachBtn} onPress={pickImageAndSend}>
                            ...
                            {inputText.trim().length > 0 && (
                                <TouchableOpacity style={styles.sendBtn} onPress={handleSendMessage}>
                                    <Ionicons name={editingMessageId ? "checkmark" : "arrow-up"} size={22}
                                              color={COLORS.white}/>
                                </TouchableOpacity>
                            )}
                        </View>
                    ...
                    <View style={styles.menuCard}>
                        {menuMessage?.type === 'text' && (
                            <TouchableOpacity style={styles.menuItem} onPress={() => {
                                ...
                                <Text style={styles.menuText}>Copy</Text><Feather name="copy" size={18}
                                                                                  color={COLORS.textMain}/>
                            </TouchableOpacity>
                        )}
                        <TouchableOpacity style={[styles.menuItem, styles.menuBorder]}
                                          onPress={() => handlePinMessage(menuMessage)}>
                            <Text style={styles.menuText}>Pin</Text><Feather name="map-pin" size={18}
                                                                             color={COLORS.textMain}/>
                        </TouchableOpacity>
                        {menuMessage?.userId === authUser?.uid && (
                            <>
                                {menuMessage?.type === 'text' && (
                                    <TouchableOpacity style={[styles.menuItem, styles.menuBorder]} onPress={() => {
                                        setEditingMessageId(menuMessage.id);
                                        setInputText(menuMessage.text);
                                        setIsMenuVisible(false);
                                    }}>
                                        <Text style={styles.menuText}>Edit</Text><Feather name="edit-3" size={18}
                                                                                          color={COLORS.textMain}/>
                                    </TouchableOpacity>
                                )}
                                <TouchableOpacity style={[styles.menuItem, styles.menuBorder]}
                                                  onPress={() => handleDeleteMessage(menuMessage.id)}>
                                    <Text style={[styles.menuText, {color: COLORS.destructive}]}>Delete</Text><Feather
                                    name="trash-2" size={18} color={COLORS.destructive}/>
                                </TouchableOpacity>
                            </>
                        )}
                    </View>
                ...
                <View style={styles.modalOverlay}>
                    <View style={styles.infoCard}>
                        ...
                        <View style={styles.infoRow}>
                            <Text style={styles.infoLabel}>Event:</Text><Text style={styles.infoValue}>{name}</Text>
                        </View>
                        <View style={styles.infoRow}>
                            <Text style={styles.infoLabel}>Time:</Text><Text
                            style={styles.infoValue}>{date} {time}</Text>
                        </View>
            ...
            <Modal visible={isPreviewVisible} transparent animationType="fade">
                <View style={styles.previewContainer}>
                    <View style={styles.previewHeader}>
                        <TouchableOpacity onPress={() => setIsPreviewVisible(false)}><Ionicons name="close" size={32}
                                                                                               color="#FFF"/></TouchableOpacity>
                        <TouchableOpacity onPress={() => saveImageToGallery(previewImage!)}><Ionicons
                            name="download-outline" size={28} color="#FFF"/></TouchableOpacity>
                    </View>
                    <ImageZoom cropWidth={SCREEN_WIDTH} cropHeight={SCREEN_HEIGHT} imageWidth={SCREEN_WIDTH}
                               imageHeight={SCREEN_HEIGHT}>
                        <Image source={{uri: previewImage || ""}} style={{width: '100%', height: '100%'}}
                               resizeMode="contain"/>
                    </ImageZoom>
                </View>
            </Modal>
...
const styles = StyleSheet.create({
    ...
    pinnedLabel: {fontSize: 10, fontWeight: '800', color: COLORS.primary, textTransform: 'uppercase'},
    ...
    authorName: {fontSize: 11, fontWeight: '700', color: COLORS.primary, marginBottom: 2},
    ...
    systemRow: {width: '100%', alignItems: 'center', marginVertical: 10},
    systemText: {fontSize: 10, fontWeight: '800', color: COLORS.textMuted, letterSpacing: 1, textAlign: 'center'},
    footerContainer: {borderTopWidth: 1, borderColor: COLORS.borderOther, padding: 12},
    inputRow: {
        flexDirection: 'row',
        alignItems: 'center',
    ...
    menuCard: {width: '75%', backgroundColor: COLORS.white, borderRadius: 20, overflow: 'hidden'},
    ...
    infoCard: {width: '85%', backgroundColor: COLORS.white, borderRadius: 28, padding: 24},
    infoTitle: {fontSize: 22, fontWeight: '800', marginBottom: 20, color: COLORS.primary},
    infoRow: {flexDirection: 'row', marginBottom: 12},
    infoLabel: {width: 60, fontWeight: '700', color: COLORS.textMuted},
    infoValue: {flex: 1, color: COLORS.textMain, fontWeight: '600'},
    clearBtn: {
    ...
    clearBtnText: {color: COLORS.destructive, fontWeight: '800'},
    closeBtn: {marginTop: 12, padding: 16, alignItems: 'center'},
