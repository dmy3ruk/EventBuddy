...
export default function FriendsScreen() {
  ...
  const handleAcceptRequest = async (req: FriendRequest) => {
    ...
    try {
      setLoading(true);
      await setDoc(doc(db, "friends", user.uid, "list", req.fromUid), { uid: req.fromUid, username: req.fromUsername, createdAt: serverTimestamp() });
      await setDoc(doc(db, "friends", req.fromUid, "list", user.uid), { uid: user.uid, username: myUsername, createdAt: serverTimestamp() });
  ...
  const handleInvitePress = async () => {
    const user = auth.currentUser;
    if (!user) return;
    const inviteRef = await addDoc(collection(db, "friendInviteLinks"), { ownerUid: user.uid, ownerUsername: myUsername, createdAt: serverTimestamp() });
